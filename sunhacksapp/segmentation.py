...
# In[Main]:
def main(img_name):
    img = cv.imread('./sample_images/'+img_name)
    try:
        global textLines, imgList, gray, letterGray, upoints, dpoints, meanu, lb, upper_baseline, lower_baseline, min_pixel_threshold, min_round_letter_threshold, min_separation_threshold, seg, wordImgList, counter, h, w, colcnt
    ...
    ###------------------------Visualize segmentation------------------------------#####        
            ...
            for i in wordImgList:
                cv.imwrite("./result/characters/" +str(counter1)+ str(counter) +".jpeg",i)
                counter=counter+1
